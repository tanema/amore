...
func BufferData(target Enum, size int, src unsafe.Pointer, usage uint32) {
	gl.BufferData(uint32(target), size, src, usage)
}
...
func VertexAttribPointer(dst Attrib, size int, ty Enum, normalized bool, stride int, offset unsafe.Pointer) {
	gl.VertexAttribPointer(uint32(dst.Value), int32(size), uint32(ty), normalized, int32(stride), offset)
}
...
// PointSize sets the GL point size
